...
static void ReadAtVirtual(OpenFile *executable, int virtualaddr, int numBytes, int position, TranslationEntry *pageTable, unsigned numPages){
  //save old 
    ...
    unsigned oldNum = machine->currentPageTableSize;
    ...
    machine->currentPageTableSize = oldNum;
...
int
AddrSpace::AllocateUserStack(int place) {
  return numPages * PageSize - (256 * place);
}

int
AddrSpace::getNumThread() {
  return numThread;
}

void
AddrSpace::incrNumThread() {
  numThread++;
}

void
AddrSpace::setNumThread( int i) {
  numThread = i;
}

void
AddrSpace::dimNumThread() {
  numThread--;
}
